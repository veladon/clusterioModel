        ...
        for (let oy = 0; oy < sy; oy++) {
            for (let ox = 0; ox < sx; ox++) {
                ...
                if (ctile && ctile != tile) {
                    ...
                    if (ctile.redirect != tile && ctile._redirectSign == ctile.redirect.size[0] + ',' + ctile.redirect.size[1])
                        return false;
                    if (reset)
                        ctile.redirect = false;
                    else
                        ctile.redirect = tile;
                }
            }
    ...
    getAdjacent(tile, includeSource) {
        ...
        for (let sY = 0; sY < sizey; sY++) {
            ...
            while (sX < sizex) {
                ...
                if (sY == 0)
                    addAdjacent(this, x + sX, y + sY, x + sX, y + sY - 1);
                if (sY == sizey - 1)
                    addAdjacent(this, x + sX, y + sY, x + sX, y + sY + 1);
                if (sX == 0)
                    addAdjacent(this, x + sX, y + sY, x + sX - 1, y + sY);
                if (sX == sizex - 1)
                    addAdjacent(this, x + sX, y + sY, x + sX + 1, y + sY);
        ...
        for (let queueIndex = 0; queueIndex < queue.length; queueIndex++) {
            ...
            if (tile && tile.size[0] > 0 && tile.size[1] > 0 && tile.state <= Connections[connectionFirstOffline][1] && done.indexOf(x + ',' + y) < 0) {
                ...
                for (let i = 0; i < adjacent.length; i++) {
                    ...
                    if (check.state >= Connections[connectionFirstOnline][0])
                        newTime = Connections[connectionFirstOffline][0];
                    // if it is a timer tile then the new time would make this tile hidden
                    else if (check.state >= Con2_0 + (timerIncrement * 3))
                        newTime = hiddenIncrement * 4;
                    else if (check.state >= Con2_0 + (timerIncrement * 2))
                        newTime = hiddenIncrement * 3;
                    else if (check.state >= Con2_0 + (timerIncrement * 1))
                        newTime = hiddenIncrement * 2;
                    else if (check.state >= Con2_0)
                        newTime = hiddenIncrement * 1;
                    // if it is a hidden tile then the new time would make this tile hidden
                    else
                        newTime = check.state - 1;
...
class Tile {
    ...
    set cache(value) {
        ...
        if (!value) {
            ...
            adjacent.forEach(tile => { tile._cache = false; if (tile.redirect)
                tile.redirect._cache = false; });
        }
    }
    // these are all just to make the tile be redrawen
    get logo() { return this._logo; }
    set logo(value) {
        if (this.size[0] * this.size[1] != 4)
            this._logo = 0;
        else
            this._logo = value;
        this.cache = false;
    ...
    set inverted(value) {
        if (this.size[0] * this.size[1] == 0 || this.state < Connections[connectionFirstTimer][0])
            this._inverted = false;
        else
    ...
    set size(value) {
        ...
        this.state = 0;
        this.logo = 0;
        if (!this.grid.setRedirects(this))
            this._size = [0, 0];
    }
    ...
    get redirect() { if (this._redirect) {
    ...
    set redirect(tile) {
        ...
        this._redirect = tile;
        if (tile)
            this._redirectSign = tile.size[0] + ',' + tile.size[1];
    }
    ...
    createBuffer() {
        ...
        let tile = false;
        Tiles.forEach(type => {
            if (type && type[0] == sizex && type[1] == sizey)
                tile = type[2];
        });
        if (this.logo > 0 && Logos[this.logo])
            tile = Logos[this.logo];
        if (!tile)
            return;
        // finds the location of the sprite on the sprite sheet
        let [sy, sx] = [0, 0];
        if (verticalSprites) {
            sy = this.state * tileSize * sizey;
            if (this.inverted)
                sx = tileSize * sizex;
        }
        else {
            sx = this.state * tileSize * sizex;
            if (this.inverted)
                sy = tileSize * sizey;
        }
        ...
        if (this.state < Connections[connectionFirstOffline][0])
            return;
        ...
        adjacent.forEach(cord => {
            ...
            for (let i = 0; i < Connections.length; i++) {
                if (connectionSelf < 0 && this.state <= Connections[i][1])
                    connectionSelf = i;
                if (connectionTarget < 0 && tile.state <= Connections[i][1])
                    connectionTarget = i;
                if (connectionSelf >= 0 && connectionTarget >= 0)
                    break;
            }
            if (!Connections[connectionSelf])
                return;
            if (connectionSelf <= 0 || connectionTarget <= 0)
                return;
...
function keyTyped() {
    ...
    switch (key) {
        case 'w':
            {
                if (tile.state <= Connections[connectionFirstOffline][1])
                    tile.state = Connections[connectionFirstTimer][0];
                else
                    tile.state++;
            }
            break;
        case 's':
            {
                if (tile.state > Connections[connectionFirstTimer][0])
                    tile.state--;
                else if (tile.state == Connections[connectionFirstTimer][0])
                    tile.state = 0;
            }
            break;
        case 'a':
        ...
        case '4':
            ...
            break;
        case 'v':
            tile.size = [tile.size[1], tile.size[0]];
            break;
        case 'p':
            save('nodeMap.png');
            break;
        ...
        case 'o':
            ...
            break;
        case 'e':
            if (tile.state == Connections[connectionFirstOnline][0]) {
                tile.state = 0;
                tile.inverted = false;
            }
            else
                tile.state = Connections[connectionFirstOnline][0];
            break;
        case 'q':
            {
                viewMode = !viewMode;
                for (let y = 0; y < grid.sizey; y++) {
                    for (let x = 0; x < grid.sizex; x++) {
                        ...
                        if (viewMode) {
                            if (grid.getTile(x, y).state >= Connections[connectionFirstTimer][0])
                                grid.setStates(grid.getTile(x, y), done);
                        }
                        else {
                            if (grid.getTile(x, y).state <= Connections[connectionFirstOffline][1])
                                grid.getTile(x, y).state = 0;
                        }
                    }
                }
            }
