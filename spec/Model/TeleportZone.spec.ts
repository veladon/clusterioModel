import { NodeInstance, Point, Grid } from "../../src/Model";
...
describe('GetTeleportZoneInFactorioCoordinates', () => {
    it('should create zones for two 1x1 nodes next to each other side-by-side left/right', () => {
        ...
        expect(node1.TeleportZones.values()[0].TopLeftCoordinate.toString()).toBe(new Point(0,0).toString());
        ...
        expect(node2.TeleportZones.values()[0].TopLeftCoordinate.toString()).toBe(new Point(1,0).toString());
    ...
    it('should create zones for two 1x1 nodes next to each other side-by-side top/bottom', () => {
        let grid = new Grid();
        let node1 = new NodeInstance('test1', new Point(0,0), 1, 1);
        let node2 = new NodeInstance('test2', new Point(0,1), 1, 1);
        grid.AddNodeToGrid(node1);
        grid.AddNodeToGrid(node2);
        ...
        expect(grid.TopLeftCoordinate.equals(new Point(0,0))).toBe(true);
        expect(grid.BottomRightCoordinate.equals(new Point(1,2))).toBe(true);
        ...
        expect(node2.TeleportZones.values().length).toBe(1);
        expect(node2.TeleportZones.keys()[0]).toBe("test1");
        expect(node2.TeleportZones.values()[0].TopLeftCoordinate.toString()).toBe(new Point(0,1).toString());
        expect(node2.TeleportZones.values()[0].GenerateTeleportZoneInFactorioCoordinates())
